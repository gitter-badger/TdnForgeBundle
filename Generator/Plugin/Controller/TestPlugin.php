...
class TestPlugin extends AbstractPlugin implements PluginInterface
{
    ...
    const FIXTURE_TYPE_ALICE = 'alice';
    const FIXTURE_TYPE_DOCTRINE = 'doctrine';
    ...
    public function isInstallable()
    {
        if (!class_exists('\Liip\FunctionalTestBundle\LiipFunctionalTestBundle')) {
            throw $this->createOptionalDependencyMissingException(
                'Please install LiipFunctionalBundle to generate tests.'
            );
        }
    ...
    private function getRelevantFixtures($directory)
    {
        switch ($this->getFixtureTypeInDirectory($directory)) {
            case self::FIXTURE_TYPE_ALICE:
                return $this->parseAliceFixtures($directory);
            case self::FIXTURE_TYPE_DOCTRINE:
